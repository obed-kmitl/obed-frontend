    ...
    <div className={styles.container}>
      ...
      >
        ...
        <div style={{ display: "flex", gap: "0.5rem" }}>
          <Tooltip
            title="กรอกครบแล้ว/ยังกรอกไม่ครบ/ยังไม่ได้กรอก"
          ...
          >
            ...
            >
              ...
              >
                {
                  stdWithScore.filter((s) => s.score_status === "Finished")
                    .length
                }
              </div>
              <div
              ...
              >
                {
                  stdWithScore.filter((s) => s.score_status === "Not Finished")
                    .length
                }
              </div>
              <div
              ...
              >
                {
                  stdWithScore.filter((s) => s.score_status === "Not Submitted")
                    .length
                }
              </div>
            </div>
      ...
      >
        ...
        {subActivity?.map((subAct, i) => (
          <Column
            title={
              <Tooltip width={300} title={subAct.detail}>{`ข้อที่ ${i + 1} (${
                subAct.max_score
              } pts)`}</Tooltip>
            }
            editable={true}
            ...
            render={(_, record) => {
              if (
                editingScore[0] === record.student_id &&
                editingScore[1] === subAct.sub_activity_id
              ) {
                return (
                  <InputNumber
                    //min={0}
                    ...
                    style={{
                      width: "100%",
                      margin: "-10px 0",
                      borderBottom: "2px solid #009fc7",
                    }}
                    onChange={(e) => onScoreChange(e, subAct.max_score)}
                    defaultValue={
                      record.scores.filter(
                        (e) => e.sub_activity_id === subAct.sub_activity_id
                      )[0]?.obtained_score
                    }
                    onBlur={() =>
                      saveScore(record.student_id, subAct.sub_activity_id)
                    }
                    onPressEnter={() =>
                      saveScore(record.student_id, subAct.sub_activity_id)
                    }
      ...
      >
        <div className={styles.importModal}>
          ...
          <div className={styles.download}>
            <Body level={3}>
              To Import Score, You need to download template and upload complete
              file to OBED.{" "}
            </Body>
            <Typography.Link
              onClick={() => {
                const modifyData = stdWithScore.map((student) => {
                  ...
                  student.scores
                    .sort((a, b) => a.sub_activity_id - b.sub_activity_id)
                    .forEach((score, i) => {
                      data[`ข้อ${i + 1}(${score.max_score})`] =
