...
export const ActivityGroup = () => {
  ...
  async function handleAddStudent(group_id, value) {
    return await httpClient
      ...
      .then(() => {
        ...
        added
          .filter((e) => e.group_id === addingGroup)[0]
  ...
  async function handleRemoveStudent(s_id, g_id) {
    return await httpClient
      ...
      .then(() => {
        let removed = [...group];
        removed.filter((e) => e.group_id === g_id)[0].students = removed
          .filter((e) => e.group_id === g_id)[0]
          .students.filter((e) => e.student_id !== s_id);
        setGroup(removed);
      })
      .catch((error) => {
        console.log(error);
  ...
  async function handleEditGroup(value) {
    return await httpClient
      ...
      .then(() => {
        let edited = [...group];
        edited.filter((g) => g.group_id === editingGroup)[0].title =
          value.title;
        setGroup(edited);
  ...
  useEffect(() => {
    form.setFieldsValue({
      title: group?.filter((g) => g.group_id === editingGroup)[0]?.title,
    });
    // eslint-disable-next-line
  }, [editingGroup]);

  useEffect(() => {
    if (section) fetchStudent();
    // eslint-disable-next-line
  }, [section]);
  
  useEffect(() => {
    if (activityId) fetchGroup();
    // eslint-disable-next-line
  }, [activityId]);
            ...
            <div className={styles.container}>
              {g.students.map((student) => (
                ...
                >
                  <div className={styles.student}>
                    ...
                    <div
                      onClick={() =>
                        handleRemoveStudent(student.student_id, g.group_id)
                      }
                    >
                      <CloseOutlined />
                    </div>
              ...
              {addingGroup === g.group_id ? (
                ...
                <div
                  ...
                  onClick={() => setAddingGroup(g.group_id)}
                >
                  Add Student
                </div>
              )}
