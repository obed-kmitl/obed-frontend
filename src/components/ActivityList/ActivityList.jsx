  ...
  return (
    <div className={styles.activityList}>
      ...
      {category.length > 0 && (
        ...
        //     </div>
        //     :
        <Collapse ghost defaultActiveKey={category.map((cat) => cat.category_id)}>
          {category.map((cat) => (
            <Panel
              header={
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <Header level={3}>{`${cat.title} (${
                    filteredActivity?.filter((atv) => atv.category_id === (cat.category_id || null))
                      .length
                  })`}</Header>
                  {cat.title !== "Unassigned" && (
                    <Header style={{ color: "#F7941D" }} level={4}>
                      {cat.weight} %
                    </Header>
                  )}
                </div>
            ...
            >
              {filteredActivity
                ?.filter((atv) => atv.category_id === (cat.category_id || null))
                .map((activity, index) => (
                  <Link
                    key={activity.activity_id}
                    to={`${window.location.pathname.split("/")[2]}/detail`}
        ...
        >
          ...
          <Form.Item label="Category" name="category_id" rules={[{ required: false }]}>
            <Select defaultValue="None" placeholder="None" allowClear>
              {category
                .filter((e) => e.category_id !== undefined)
                .map(
                  (cat) => (
                    <Option key={cat.category_id} value={cat.category_id}>
                      {cat.title}
                    </Option>
                  ) //Unassigned Activity => id 0
                )}
            </Select>
          </Form.Item>
        </Form>
